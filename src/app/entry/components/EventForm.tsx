...
export default function EventForm() {
    ...
    const submitEventMutation = async (data: Record<string, unknown>) => {
    ...
    // DEV_MODE (isDevelopment=true) ignores period restrictions
    ...
    function validate() {
        ...
        const requiredMembers = teamSize;
        ...
        const effectiveLeaderName = leaderName.trim() || memberLeaderName || "";
        const effectiveLeaderEmail = leaderEmail || (leaderIndex >= 0 && members[leaderIndex]?.studentId ? `c${members[leaderIndex].studentId}@st.kanazawa-it.ac.jp` : "");
    ...
    async function onSubmit(e: React.FormEvent) {
        ...
        try {
            await submitEventMutation(collected as unknown as Record<string, unknown>);
    ...
    if (afterEnd) {
        return (
            <div className="w-full max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
                <div className="p-8 bg-white rounded-xl shadow-sm border border-[#FFE8D6] text-center">
                    <h2 className="text-2xl font-bold text-[#2d2d2d] mb-4">申し込みは終了しました</h2>
                    <p className="text-[#cc5226] mb-6">ご参加いただきありがとうございました。</p>
                </div>
            </div>
        );
    }
        ...
        <form onSubmit={onSubmit} className="w-full max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div className="mb-8 text-center">
                ...
                {settings && settings.enabled === false && (
                    <div className="mb-2 text-sm text-[#cc5226]">開発環境: 運営設定は無効です</div>
                )}
                {!isDevelopment && appEnd && (
                    <Countdown targetDate={appEnd} title="申し込み終了まで" />
                )}
                <h2 className="text-3xl sm:text-4xl font-bold mb-2 text-[#2d2d2d]">HacKit 2026</h2>
                <h3 className="text-xl sm:text-2xl font-semibold mb-4 text-[#b34720]">イベント申し込みフォーム</h3>
                <p className="text-[#cc5226] font-medium">繋がる、創る、超えていく。</p>
            </div>

            {/* Basic Information Section */}
            <section className="mb-8 bg-white bg-opacity-60 rounded-lg p-6 border-2 border-[#FFD4B0]">
                <h3 className="text-xl font-bold mb-4 text-[#2d2d2d] flex items-center">
                    <span className="bg-[#FF6B35] text-white rounded-full w-8 h-8 flex items-center justify-center mr-3 text-sm font-bold">1</span>
                    基本情報
                </h3>
                <label className="block mb-4">
                    <span className="block text-[#2d2d2d] font-semibold mb-2">所属プロジェクト名 <span className="text-red-500">*</span></span>
                    <input
                        className="w-full border-2 border-[#FFB380] rounded-lg p-3 focus:outline-none focus:border-[#FF6B35] focus:ring-2 focus:ring-[#FFD4B0]"
                        value={projectName}
                        onChange={(e) => setProjectName(e.target.value)}
                        disabled={noAffiliation}
                        placeholder="例：データサイエンスプロジェクト"
                    />
                    <label className="inline-flex items-center mt-2 text-[#2d2d2d]">
                        <input
                            type="checkbox"
                            ...
                            className="w-4 h-4 mr-2 accent-[#FF6B35]"
                        />
                        <span>無所属</span>
                    </label>
                ...
                <label className="block">
                    ...
                    <select
                        className="border-2 border-[#FFB380] rounded-lg p-3 focus:outline-none focus:border-[#FF6B35] focus:ring-2 focus:ring-[#FFD4B0]"
                        value={teamSize}
            ...
            <section className="mb-8 bg-white bg-opacity-60 rounded-lg p-6 border-2 border-[#FFD4B0]">
                <h3 className="text-xl font-bold mb-4 text-[#2d2d2d] flex items-center">
                    <span className="bg-[#FF6B35] text-white rounded-full w-8 h-8 flex items-center justify-center mr-3 text-sm font-bold">2</span>
                    チームメンバー情報
                </h3>
                {Array.from({ length: teamSize }).map((_, idx) => (
                    <div key={idx} className="mb-6 p-4 bg-[#FFF5EE] rounded-lg border border-[#FFD4B0]">
                        <div className="flex items-center justify-between mb-4">
                            <h4 className="font-bold text-[#2d2d2d]">{idx + 1}人目 <span className="text-red-500">*</span></h4>
                            <label className="flex items-center text-[#2d2d2d]">
                                <input
                                    type="checkbox"
                                    ...
                                    className="w-5 h-5 mr-2 accent-[#FF6B35]"
                                />
                                <span>リーダーにチェック</span>
                            </label>
                        </div>
                        <label className="block mb-3">
                            <span className="block text-[#b34720] font-semibold mb-2">学年・学科・クラス</span>
                            <input
                                className="w-full border-2 border-[#FFB380] rounded-lg p-3 focus:outline-none focus:border-[#FF6B35] focus:ring-2 focus:ring-[#FFD4B0]"
                        ...
                        <label className="block mb-3">
                            <span className="block text-[#b34720] font-semibold mb-2">学籍番号</span>
                            <input
                                className="w-full border-2 border-[#FFB380] rounded-lg p-3 focus:outline-none focus:border-[#FF6B35] focus:ring-2 focus:ring-[#FFD4B0]"
                        ...
                        <label className="block">
                            <span className="block text-[#b34720] font-semibold mb-2">氏名（姓 / 名）</span>
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                                <input
                                    className="flex-1 border-2 border-[#FFB380] rounded-lg p-3 focus:outline-none focus:border-[#FF6B35] focus:ring-2 focus:ring-[#FFD4B0]"
                            ...
                            <label className="block mt-3">
                                <span className="block text-[#b34720] font-semibold mb-2">フリガナ（姓 / 名）</span>
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                                    <input
                                        className="flex-1 border-2 border-[#FFB380] rounded-lg p-3 focus:outline-none focus:border-[#FF6B35] focus:ring-2 focus:ring-[#FFD4B0]"
                            ...
                            <label className="block mt-3">
                                <span className="block text-[#b34720] font-semibold mb-2">GitHub URL（任意）</span>
                                <input
                                    className="w-full border-2 border-[#FFB380] rounded-lg p-3 focus:outline-none focus:border-[#FF6B35] focus:ring-2 focus:ring-[#FFD4B0]"
                            ...
                            <div className="mt-3">
                                <span className="block text-[#b34720] font-semibold mb-2">性別（任意）</span>
            ...
            <section className="mb-8 bg-white bg-opacity-60 rounded-lg p-6 border-2 border-[#FFD4B0]">
                <h3 className="text-xl font-bold mb-4 text-[#2d2d2d] flex items-center">
                    <span className="bg-[#FF6B35] text-white rounded-full w-8 h-8 flex items-center justify-center mr-3 text-sm font-bold">3</span>
                    チーム情報
                </h3>
                <label className="block mb-4">
                    <span className="block text-[#2d2d2d] font-semibold mb-2">チームリーダーの名前 <span className="text-red-500">*</span></span>
                    <input
                        className="w-full border-2 border-[#FFB380] rounded-lg p-3 bg-gray-100 text-gray-700 opacity-90 cursor-not-allowed"
                        value={leaderName}
                ...
                <label className="block mb-4">
                    <span className="block text-[#2d2d2d] font-semibold mb-2">チームリーダーのメールアドレス <span className="text-red-500">*</span></span>
                    <input
                        className="w-full border-2 border-[#FFB380] rounded-lg p-3 bg-gray-100 text-gray-700 opacity-90 cursor-not-allowed"
                        value={leaderEmail}
                ...
                <p className="mb-4 text-[#b34720]">チームメンバー一覧で、該当メンバーに「リーダーにチェック」を付けてください。</p>
                <label className="block">
                    <span className="block text-[#2d2d2d] font-semibold mb-2">チームの説明・アイデア概要</span>
                    <textarea
                        className="w-full border-2 border-[#FFB380] rounded-lg p-3 focus:outline-none focus:border-[#FF6B35] focus:ring-2 focus:ring-[#FFD4B0] min-h-24"
            ...
            <section className="mb-8 bg-white bg-opacity-60 rounded-lg p-6 border-2 border-[#FFD4B0]">
                <h3 className="text-xl font-bold mb-4 text-[#2d2d2d] flex items-center">
                    <span className="bg-[#FF6B35] text-white rounded-full w-8 h-8 flex items-center justify-center mr-3 text-sm font-bold">4</span>
                    参加条件
                </h3>
                <div className="mb-4 p-3 bg-[#FFF5EE] rounded-lg border border-[#FFD4B0]">
                    <p className="text-[#2d2d2d] font-semibold mb-3">
                    ...
                    <div className="flex items-center gap-4">
                        <label className="flex items-center cursor-not-allowed">
                            ...
                            />
                            <span className="ml-2 text-[#2d2d2d]">はい</span>
                        </label>
                        <label className="flex items-center cursor-not-allowed">
                            <input
                                type="radio"
                                ...
                                className="w-4 h-4 bg-gray-100 text-gray-700 cursor-not-allowed"
                            />
                            <span className="ml-2 text-[#2d2d2d]">いいえ</span>
                        </label>
                    </div>
                </div>
            </section>
            ...
            <section className="mb-8 bg-white bg-opacity-60 rounded-lg p-6 border-2 border-[#FFD4B0]">
                <h3 className="text-xl font-bold mb-4 text-[#2d2d2d] flex items-center">
                    <span className="bg-[#FF6B35] text-white rounded-full w-8 h-8 flex items-center justify-center mr-3 text-sm font-bold">5</span>
                    同意事項
                </h3>
                <p className="text-[#2d2d2d] font-semibold mb-4">以下の項目すべてにチェックしてください <span className="text-red-500">*</span></p>
                <div className="space-y-3">
                    <label className="flex items-start cursor-pointer p-3 hover:bg-[#FFF5EE] rounded-lg transition">
                        <input
                            type="checkbox"
                            checked={agreeCancel}
                            onChange={(e) => setAgreeCancel(e.target.checked)}
                            className="w-5 h-5 mt-1 accent-[#FF6B35] flex-shrink-0"
                        />
                        <span className="ml-3 text-[#2d2d2d]">申し込み後のキャンセルはできません。</span>
                    </label>
                    <label className="flex items-start cursor-pointer p-3 hover:bg-[#FFF5EE] rounded-lg transition">
                        <input
                            type="checkbox"
                            ...
                            className="w-5 h-5 mt-1 accent-[#FF6B35] flex-shrink-0"
                        />
                        <span className="ml-3 text-[#2d2d2d]">会場や活動の様子を撮影し、広報用のSNS等に掲載する場合があります。また、Zoomで録音・録画を行うことがあります。</span>
                    </label>
                    <label className="flex items-start cursor-pointer p-3 hover:bg-[#FFF5EE] rounded-lg transition">
                        <input
                            type="checkbox"
                            ...
                            className="w-5 h-5 mt-1 accent-[#FF6B35] flex-shrink-0"
                        />
                        <span className="ml-3 text-[#2d2d2d]">スポンサー企業に氏名およびメールアドレス等の連絡先情報を共有する場合があります。</span>
                    </label>
                    <label className="flex items-start cursor-pointer p-3 hover:bg-[#FFF5EE] rounded-lg transition">
                        <input
                            type="checkbox"
                            ...
                            className="w-5 h-5 mt-1 accent-[#FF6B35] flex-shrink-0"
                        />
                        <span className="ml-3 text-[#2d2d2d]">応募者が多数の場合、参加者を抽選で決定することがあります。</span>
                    </label>
                </div>
            </section>
            ...
            <section className="mb-8 bg-white bg-opacity-60 rounded-lg p-6 border-2 border-[#FFD4B0]">
                <h3 className="text-xl font-bold mb-4 text-[#2d2d2d] flex items-center">
                    <span className="bg-[#FF6B35] text-white rounded-full w-8 h-8 flex items-center justify-center mr-3 text-sm font-bold">6</span>
                    アレルギー情報
                </h3>
                <div className="mb-4 p-3 bg-[#FFF5EE] rounded-lg border border-[#FFD4B0]">
                    <p className="text-[#2d2d2d] font-semibold mb-3">食物アレルギーの有無</p>
                    <p className="text-[#e55e2b] text-sm mb-2">※イベント最終日の懇親会で食事が出ます。</p>
                    <div className="flex items-center gap-4">
                ...
                {hasAllergy === "yes" && (
                    <label className="block">
                        <span className="block text-[#2d2d2d] font-semibold mb-2">アレルギーのある方の名前と対象の食物</span>
                        <textarea
                            className="w-full border-2 border-[#FFB380] rounded-lg p-3 focus:outline-none focus:border-[#FF6B35] focus:ring-2 focus:ring-[#FFD4B0] min-h-20"
                            value={allergyDetail}
            ...
            <div className="flex flex-col sm:flex-row gap-4 mb-8">
                <button
                    type="button"
                    onClick={openPreview}
                    className="w-full sm:flex-1 bg-white border-2 border-[#FF8C5A] text-[#2d2d2d] font-bold py-3 px-6 rounded-lg hover:bg-[#FFF5EE] transition duration-200"
                >
                    入力内容をプレビュー
                </button>
                <button
                    ...
                    className={`w-full sm:flex-1 ${isSubmitting ? "bg-[#FFB380] cursor-not-allowed" : "bg-[#FF6B35] hover:bg-[#e55e2b]"} text-white font-bold py-3 px-6 rounded-lg transition duration-200 transform hover:scale-105 shadow-lg`}
                >
            ...
            {showPreview && (
                <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50">
                    <div className="bg-white p-6 rounded-lg w-11/12 max-w-2xl max-h-[90vh] overflow-y-auto">
                        <h3 className="text-xl font-bold mb-4 text-[#2d2d2d]">入力内容を確認</h3>
                        <div className="space-y-2 text-[#2d2d2d] text-sm">
                            ...
                            <p><strong>リーダー名:</strong> {leaderName || '\u2014'}</p>
                            <p><strong>リーダーメール:</strong> {leaderEmail || '\u2014'}</p>
                            <p><strong>1年生含む:</strong> {hasFirstYear === 'yes' ? 'はい' : 'いいえ'}</p>
                            <p><strong>チーム説明:</strong> {teamDescription || '\u2014'}</p>
                        ...
                        <div className="flex justify-end gap-3 mt-6">
                            <button onClick={() => setShowPreview(false)} className="bg-white border-2 border-[#FF8C5A] text-[#2d2d2d] font-bold py-2 px-4 rounded-lg hover:bg-[#FFF5EE]">編集する</button>
                            <button onClick={handleConfirmSubmit} className="bg-[#FF6B35] text-white font-bold py-2 px-4 rounded-lg hover:bg-[#e55e2b]">送信する</button>
                        </div>
                    </div>
                </div>
            )}
