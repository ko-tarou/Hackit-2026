    ...
    >
      ...
      <div className="max-w-5xl mx-auto">
        <FadeInText className="mb-16">
          <span className="text-sm font-medium tracking-[0.3em] text-accent mb-4">スケジュール</span>
          <h2 className="text-4xl md:text-5xl lg:text-6xl font-bold text-base-dark">スケジュール</h2>
        </FadeInText>

        <StaggerContainer staggerChildren={0.1} delayChildren={0.1}>
          <div className="relative">
            {/* スクロールで伸びるタイムライン（SVG pathLength） */}
            <div className="absolute left-6 md:left-8 top-0 bottom-0 w-0.5 bg-base-dark/10 rounded-full overflow-hidden">
              <motion.div
            ...
            <div className="space-y-0">
              {SCHEDULE_ITEMS.map((item) => (
                <motion.div
                  ...
                  className="relative pl-16 md:pl-24 pb-14 last:pb-0 group"
                >
                  <div className="absolute left-4 md:left-6 top-1 w-3 h-3 rounded-full bg-accent group-hover:scale-150 group-hover:ring-4 group-hover:ring-accent/30 transition-all -translate-x-1/2 z-10" />
                  <div className="flex flex-col md:flex-row md:items-baseline gap-2 md:gap-8">
                    <time dateTime={item.date.replace(/\./g, "-")} className="text-sm font-bold text-accent tracking-wider">{item.date}</time>
                    <div className="flex-1">
                      <h3 className="text-xl md:text-2xl font-bold text-base-dark mb-1 group-hover:text-accent transition-colors">
                        {item.title}
                      </h3>
                      <p className="text-base-dark/70">{item.description}</p>
                    </div>
                  </div>
                </motion.div>
              ))}
