...
const SPONSORS_PUBLIC = false;

const SPONSOR_TIERS = [
  { tier: "プラチナスポンサー", tierEn: "Platinum", count: 4, className: "w-56 h-28 md:w-72 md:h-36 flex-shrink-0", scroll: false },
          ...
          <div className="relative overflow-hidden rounded-2xl h-[200px] sm:h-[240px]">
            ...
            >
              <div>
                ...
                <div className="flex flex-wrap gap-4 md:gap-6">
                  {Array.from({ length: SPONSOR_TIERS[0].count }).map((_, i) => (
                    <SponsorPlaceholder key={i} className={SPONSOR_TIERS[0].className} />
                  ))}
                </div>
              </div>
            </div>
